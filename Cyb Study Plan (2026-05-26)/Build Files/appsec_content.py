...
def build_content(story, S, banner, bi, nb, rule, grid_table,
                  ...
                  TA_JUSTIFY, TA_CENTER, TA_RIGHT, inch):
    ...
    toc = [
        ...
        ("VIII \u2014 Technical Interview Preparation", "42",
            ["Interview Format", "Conceptual Questions", "Framing Your Background"]),
        ("VIII \u2014 Expected First Days on the Job", "45",
    ...
    for para in [
        ...
        "security misconfigurations, insecure deserialization \u2014 these are not abstract "
        ...
        "lives at the HTTP layer, in APIs, in authentication flows, in the browser. Software "
        "developers who have worked in this stack have context that pure security specialists "
        "lack \u2014 they understand for five years. The difference is the perspective.",
    ]:
    ...
    add(P("The application layer is where the majority of those vulnerabilities live. More than "
          ...
          "lifecycle \u2014 which means over 40% still do not, and most of those companies are "
    ...
    add(P("The global application security market was valued at $13.68 billion in 2024 and is "
          ...
          "postings shows Application Security Engineers earning $126,000\u2013$194,000 at "
    ...
    SS("Why your background fits this moment specifically.")
    add(P("AppSec requires two things that rarely coexist: genuine software engineering depth and "
          "adversarial security thinking. The market is not short on people with security "
          "certifications. It is short on people who can read a FastAPI codebase, identify where "
          "user input reaches an interpreter without sanitization, explain the business impact of "
          "the finding, and help the development team fix it correctly. You have spent five years "
          "on the first half of that equation. This plan builds the second half.", S["body"]))
    add(SP(4))
    ...
    add(P("This plan is a deliberate pivot, not a hedge. You spent five years building the attack "
          ...
          "frontends, AWS infrastructure, Docker containers \u2014 the exact technology stack that "
    ...
    add(B("<b>Testing discipline</b> \u2014 You wrote unit, integration, and regression tests. "
          "AppSec testing follows the same logic: inputs, expected behavior, failure modes."))
    add(SP(2))
    SS("No LeetCode")
    add(P("AppSec interviews test security knowledge \u2014 OWASP concepts, secure code review, "
          ...
          "code makes you structurally prepared for. The relief this should produce is real and "
          "earned.", S["body"]))
    SS("The Timing Argument")
    add(P("The 2026 job market is difficult for web development. It is not difficult for "
          "security. AI is augmenting Tier 1 SOC analyst work, but it is not replacing AppSec "
          "engineers who can find subtle authentication bypasses in complex codebases, threat "
          "model a new microservice architecture, and communicate risk to a VP of Engineering. "
    ...
    add(P("Web application development and application security share the same foundation: HTTP, "
          ...
          "in a FastAPI application is an AppSec concern. Every authentication flow is a potential "
    ...
    add(SP(6))
    add(nb("Start applying at Month 3\u20134 for AI-native startups and security-conscious tech "
    ...
    add(P("This plan is calibrated for 5\u20137 months at 20\u201323 study hours per week. 5 "
          "months is the optimistic case; 7 months is the outer bound; 6 months is the realistic "
          "midpoint. No math prerequisites. The hardest phase is not technical \u2014 it is "
    ...
    sched = [
        ...
        [P("Long Study", S["tc"]), P("3", S["tcc"]), P("6\u20137 hrs", S["tcc"]),
         P("Primary reading, labs, implementation, tool practice", S["tc"])],
        [P("Rest w/ Short Study", S["tc"]), P("1", S["tcc"]), P("2.5 hrs", S["tcc"]),
         P("Note card review, writeup drafting, light lab work", S["tc"])],
        [P("<b>TOTAL STUDY</b>", S["tc"]), P("<b>4</b>", S["tcc"]), P("<b>20\u201323 hrs</b>", S["tcc"]),
         P("Per week", S["tc"])],
        [P("Work (Gigs)", S["tc"]), P("2", S["tcc"]), P("6\u20137 hrs", S["tcc"]),
         P("Income \u2014 no study obligation on work days", S["tc"])],
        [P("Full Rest", S["tc"]), P("1", S["tcc"]), P("\u2014", S["tcc"]),
         P("No study, no work. Required for retention.", S["tc"])],
    ]
    ...
    names = ["Pre-\nStudy","AppSec\nIntro","Web &\nNetwork","Core\nVulns","Testing &\nTooling",
             "Cloud\nSecurity","Code\nReview","Sec+\nCert","Portfolio","Inter-\nviews","First\nDays"]
    weeks = ["Wks 1\u20133","Wks 4\u20135","Wks 6\u20139","Wks 10\u201314","Wks 15\u201318",
             "Wks 19\u201320","Wks 21\u201323","Wks 24\u201326","Mo 6.5","Mo 6.5\u20137","Mo 6+"]
    tl_colors = ["#2C3E50","#1B4F72","#1A5276","#154360","#0E6655","#145A32",
                 "#6E2F1A","#6E2F3E","#4A235A","#2E4057","#212F3C"]
    ...
    cw = avail/11.0
    tt = Table(tdata, colWidths=[cw]*11, rowHeights=[24, 38, 24])
    ts = [
    ...
    add(tt)
    add(SP(8))
    add(nb("<b>Timeline:</b> 5 months is the optimistic case; 7 months is the outer bound; 6 "
           "months is the realistic midpoint. The first 2\u20133 weeks are pre-study before "
           "security content begins. Start applying at Month 4\u20135 for startups and "
           "AppSec-adjacent roles. Sec+ appears on your resume at Month 6 \u2014 before the main "
           "application push. On the two-year employment gap: \u2018I have been in a deliberate "
           "full-time transition into application security since May 2026. This plan is the "
           "evidence.\u2019"))
    ...
    SS("Human Mentorship: MentorCruise")
    ...
    add(B("<b>Target:</b> 4\u20136 published writeups before Month 5 applications. Quality over "
          "quantity."))
    add(B("<b>GitHub:</b> Scripts from the tooling phase, custom Semgrep rules, automation tools. "
          "Commit history from Month 2 onward. Clean READMEs."))
    add(SP(2))
    SS("Application Strategy")
    add(B("<b>Start applying at Month 5\u20136:</b> Security-conscious startups are viable "
          "targets from Month 5 with Phases I\u2013III complete, two published writeups, and "
    ...
    add(B("<b>Frame the gap explicitly:</b> Cover letters should state upfront: \u2018After five "
          ...
          "security in May 2026.\u2019 This plan is the evidence."))
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    H("Philadelphia AppSec Community")
    ...
    res("Existing Docker Notes", "Personal notes covering containers, images, Dockerfiles, "
    ...
    step("01", "READ + BUILD", "FastAPI Documentation \u2014 Complete \u00b7 Weeks 1\u20133", [
        ...
        "builds the complete mental model needed to audit FastAPI codebases.",
    ...
    step("02", "REVIEW", "HTTP & Cookies \u00b7 Days 1\u20133", [
        "<b>HTTP:</b> Review existing HTTP notes (HTML intro notes and Odin HTTP notes). These "
    ...
    step("03", "LEARN + PRACTICE", "Docker \u00b7 Days 2\u20134", [
    ...
    step("04", "SETUP", "Git & GitHub Hygiene \u00b7 Day 1", [
    ...
    concept("Cookie security attributes",
            ...
            "security controls. Their absence or misconfiguration is a finding. Their absence or "
            "misconfiguration is a finding in any security review.")
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("I", "AppSec Intro: A Dip into Offense", "Weeks 4\u20135 \u00b7 Month 1"))
    add(SP(10))
    add(P("AppSec has a mysticism problem from the opposite direction of AI: where AI hype makes "
          "it seem unknowably complex, security culture sometimes makes it seem inaccessible "
          "without a decade of CTF grinding. Neither is true, and this phase exists to "
          "demonstrate it directly \u2014 before any systematic study begins.", S["body"]))
    add(P("The goal is not to become a penetration tester in three weeks. It is to arrive at the "
    ...
    res("TryHackMe", "tryhackme.com \u2014 Browser-based, beginner-friendly, guided rooms. The "
        "\u2018Web Fundamentals\u2019 path covers HTTP, Burp Suite basics, OWASP Top 10, and web "
        "vulnerability basics. The \u2018Jr Penetration Tester\u2019 path covers web "
        "vulnerabilities, Burp Suite, and penetration testing methodology.")
    res("Real-World Bug Hunting", "Peter Yaworski, No Starch Press, 2019 (nostarch.com) \u2014 A "
    ...
    step("01", "EXPLORE", "PortSwigger Academy Orientation \u00b7 Week 4", [
        "Create a free PortSwigger account. Work through the \u2018Server-side topics\u2019 "
        "overview",
        "Complete the SQL Injection introductory labs (first 3\u20134)",
        "Complete the XSS introductory labs (first 3\u20134)",
    ])
    step("02", "READ", "Real-World Bug Hunting Ch. 1\u20135 \u00b7 Weeks 4\u20135", [
    ...
    step("04", "EXPLORE", "TryHackMe Orientation \u00b7 Week 5", [
        "Complete the \u2018Web Fundamentals\u2019 path: HTTP in Detail, Burp Suite basics, OWASP "
        "Top 10 room",
    ...
    concept("The OWASP Top 10",
            ...
            "list exists and has structure. Mastery of each class comes through dedicated study.")
    concept("Injection as a class",
            "Any time user input reaches an interpreter without proper sanitization, injection is "
            "possible. SQL is the canonical example. Command injection, LDAP injection, and "
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("II", "Web & Network Foundations", "Weeks 6\u20139 \u00b7 Month 1.5\u20132"))
    add(SP(10))
    add(P("This phase builds the technical foundation for AppSec work. Every major vulnerability "
          ...
          "\u2014 not just how they work, but why they fail. It closes with two focused study "
          "weeks: cryptography fundamentals and the specific networking knowledge that appears in "
          "AppSec interviews.", S["body"]))
    add(SP(2))
    SS("Learning Resources")
    res("Alice and Bob Learn Application Security", "Tanya Janca (SheHacksPurple), Wiley, 2020 "
    ...
    res("The Web Application Hacker\u2019s Handbook", "Dafydd Stuttard and Marcus Pinto, Wiley, "
        "2011 (wiley.com) \u2014 Foundational reference written by the creator of Burp Suite. Ch. "
        "1\u20133 cover web application fundamentals and core protocols. Ch. 6 covers "
    ...
    res("TryHackMe \u2018Pre-Security\u2019 path \u2014 Network Fundamentals module",
        "tryhackme.com \u2014 Primary resource for the networking week. Covers OSI model, TCP/IP, "
    ...
    res("TryHackMe \u2018Pre-Security\u2019 path \u2014 Putting It All Together room",
        "tryhackme.com \u2014 Covers how all the components of a web request fit together: DNS, "
    ...
    add(SP(4))
    SS("Web & Network Foundations \u2014 Weeks 6\u20139")
    step("01", "READ + LABS", "HTTP Deep Dive \u00b7 Week 6", [
        "Read WAHH Ch. 1\u20133 (web application security, core protocols, web application "
        "technologies) as foundational context",
        "Work through PortSwigger \u2018HTTP fundamentals\u2019 and \u2018Burp Suite\u2019 "
        "introductory material \u2014 these are the primary sources for this step",
        "<b>Full notes + note cards:</b> CORS misconfigurations, CSP, TLS, HTTP methods and "
        "status codes with security implications",
    ])
    step("02", "READ + LABS", "Authentication & Session Management \u00b7 Weeks 6\u20137", [
        ...
        "Complete PortSwigger \u2018Authentication\u2019 learning path \u2014 all apprentice and "
        "practitioner labs",
        "<b>Full notes + note cards:</b> Password enumeration, session fixation, JWT algorithm "
        "confusion, OAuth redirect URI manipulation",
    ])
    step("03", "PROJECT", "Authentication Audit Checklist \u00b7 Week 5", [
    ...
    step("05", "READ + LABS", "Input Handling Fundamentals \u00b7 Weeks 7\u20138", [
        "Complete PortSwigger \u2018SQL injection\u2019 learning path completely (apprentice "
        "through expert)",
        "<b>Full notes + note cards:</b> SQL injection root cause, parameterized queries, "
        "injection taxonomy",
    ])
    step("06", "READ", "Alice and Bob Learn Application Security \u00b7 Week 8", [
        "Read Alice and Bob Learn Application Security Ch. 1\u20135: security requirements, secure "
        "design concepts, common pitfalls, AppSec program basics",
        "Read during this week \u2014 this is the defender and program-building perspective that "
        "PortSwigger and WAHH do not cover",
    ...
    step("07", "STUDY", "Cryptography Fundamentals \u00b7 Week 9", [
        ...
        "Read the OWASP Cryptographic Storage Cheat Sheet and the OWASP Transport Layer Security "
        "Cheat Sheet. Take notes.",
        "<b>Hands-on:</b> Crack an MD5 hash with hashcat. Implement bcrypt password hashing in "
        "Python. These make the concepts concrete.",
    ...
    step("08", "STUDY", "Networking Fundamentals for AppSec \u00b7 Week 9", [
        "Complete the TryHackMe \u2018Pre-Security\u2019 path \u2014 Network Fundamentals module. "
        "Take notes.",
        "Review existing Odin Project HTTP notes and HTML intro notes for the URL-to-response "
        "flow.",
    ...
    concept("API security",
            "REST API-specific vulnerability patterns. REST API-specific vulnerability patterns "
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("III", "Core Vulnerability Classes", "Weeks 10–14 · Month 2.5–3.5"))
    add(SP(10))
    ...
    res("Alice and Bob Learn Application Security", "Tanya Janca (SheHacksPurple), Wiley, 2020 "
        ...
        "an AppSec program.")
    res("Hacking APIs", "Corey Ball, No Starch Press, 2022 (nostarch.com) — REST API security "
        "testing. Ch. 1–3 and Ch. 6–9 cover API reconnaissance, endpoint discovery, and "
        "authentication testing.")
    add(SP(4))
    SS("Core Vulnerability Classes — Weeks 10–14")
    step("01", "LABS: Injection Classes", "Weeks 10–11", [
        ...
        "For each lab: exploit it, write your standard documentation (what, why, fix, code review "
        "pattern)",
    ])
    step("02", "LABS: Cross-Site Scripting", "Week 11", [
        "Complete PortSwigger XSS learning path: reflected, stored, and DOM XSS as distinct "
        "categories",
        "XSS in React: React’s JSX escaping prevents many reflected XSS vectors by default. The "
    ...
    step("03", "LABS: CSRF, SSRF, XXE", "Weeks 11–12", [
        "Complete PortSwigger CSRF, SSRF, and XXE learning paths",
        "SSRF: a vulnerability where an attacker tricks a server into making HTTP requests to "
    ...
    step("04", "LABS: Access Control & Business Logic", "Week 12", [
    ...
    step("05", "LABS: Deserialization, File Upload, Path Traversal", "Week 13", [
    ...
    step("06", "READ + LABS: API Security Deep Dive", "Weeks 13–14", [
        ...
        "Exposure",
        "Read Hacking APIs Ch. 1–3 (API reconnaissance, endpoint discovery, authentication "
        "testing) and Ch. 6–9",
        "Complete PortSwigger API testing learning path",
        "Work through it treating your own past FastAPI work as the mental target: would the APIs "
        "you built have been vulnerable?",
    ])
    step("07", "PROJECT: OWASP Top 10 Writeup Series", "Throughout Weeks 10–14", [
        "One 800–1,200 word writeup per OWASP Top 10 category, published progressively throughout "
        "this phase",
    ...
    concept("XSS — all three types",
            ...
            "Fix: context-appropriate output encoding. Code review: any innerHTML assignment, "
    ...
    concept("CSRF and SSRF",
            ...
            "Fix: SameSite cookies for CSRF; URL allowlists for SSRF.")
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("IV", "Security Testing & Tooling", "Weeks 15–18 · Month 3.5–4.5"))
    add(SP(10))
    add(P("Phase IV moves from knowing vulnerability classes to operating the professional "
          ...
          "(Semgrep) and DAST are the automation layer. The goal is to reach a level of tool "
    ...
    res("Burp Suite Documentation", "portswigger.net/burp/documentation — Official documentation "
        "for all Burp Suite tools: Proxy, Repeater, Intruder, Sequencer, and Scanner. Burp Suite "
    ...
    res("OWASP ZAP Documentation", "zaproxy.org/docs — DAST tool. The open-source alternative to "
        "Burp Suite Pro for automated scanning.")
    ...
    res("Gitleaks", "github.com/gitleaks/gitleaks — Lightweight secrets scanner. Runs as a "
        ...
        "for every PR.")
    add(SP(4))
    SS("Security Testing & Tooling — Weeks 15–18")
    step("01", "LEARN + LABS: Burp Suite Mastery", "Weeks 15–16", [
        "Install Burp Suite Community Edition. Work through the official documentation: Proxy, "
        "Repeater, Intruder, Decoder, Comparer",
        "Work through the PortSwigger ‘Burp Suite’ learning path labs completely",
        "By end of Week 15: intercept and modify any HTTP request, use Repeater to test "
        "hypotheses, use Intruder for parameter fuzzing",
    ])
    step("02", "PROJECT: Burp Suite Certified Practitioner Prep", "Weeks 15–17", [
        "PortSwigger offers the BSCP exam — free to take, highly respected in the AppSec "
        "community, practical not multiple-choice",
    ...
    step("03", "EXAM: Burp Suite Certified Practitioner", "Week 17–18", [
        "Take the BSCP exam. The exam consists of two applications, each requiring two "
        ...
        "retake. The exam can be retaken.",
    ])
    step("05", "LEARN + LABS: Semgrep & Bandit — SAST Tooling", "Week 16–17", [
        "Install Semgrep CLI. Run the default Python ruleset against a sample Python application "
        "(OWASP Juice Shop or WebGoat codebase)",
        "Triage the results: distinguish true positives from false positives",
        "Write one custom Semgrep rule targeting a Python injection pattern — for example, an "
        ...
        "Bandit: install with pip install bandit. Run against a sample Python project and against "
        "your own old code if available. Look for: B101 (assert usage), B102 (exec usage), "
    ...
    step("05", "LEARN: Secrets Scanning — TruffleHog & Gitleaks", "Week 17", [
        ...
        "TruffleHog: install and run against a sample repo. Uses entropy analysis to catch secrets "
        "that regex misses. Can scan full git history, not just current code",
        "Gitleaks: install and run as a CI pre-commit hook. Lightweight and fast, well-suited for "
        "blocking commits before they land",
        "Integrate both into a GitHub Actions workflow: Gitleaks on every pull request (scanning "
        "only the changed lines), TruffleHog for scheduled full-history scans",
        "In code review: you are now looking for hardcoded strings that look like keys, passwords, "
        "or tokens — in source code, config files, Dockerfiles, and environment variable defaults",
        "Full notes + note cards: When to use TruffleHog vs Gitleaks, what a false positive looks "
    ...
    step("07", "LEARN: Snyk — Dependency Scanning", "Week 17–18", [
    ...
    step("08", "PROJECT: Full Security Assessment of Juice Shop", "Week 18", [
        "Conduct a full security assessment of OWASP Juice Shop using: Burp Suite (manual), ZAP "
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("V", "Cloud Security Fundamentals", "Weeks 19–20 · Month 4.5–5"))
    add(SP(10))
    ...
    add(SP(4))
    SS("Cloud Security Fundamentals — Weeks 19–20")
    step("01", "READ: AWS Foundations: Shared Responsibility & IAM", "Week 19", [
        "Read the AWS Shared Responsibility Model page completely. Write a one-paragraph summary "
        "in your own words: what AWS is responsible for, what you are responsible for, and what "
        "the boundary looks like for a FastAPI application running on EC2 behind an S3 bucket",
        "Read the IAM Concepts section in AWS documentation: what a user is, what a role is, what "
        "a policy is, how they attach, what least privilege means in practice",
        "Read the FreeCodeCamp Cloud Security Fundamentals in AWS guide — beginner-friendly, "
        "hands-on, covers IAM and S3 configuration with real console examples",
        "Full notes + note cards: AWS Shared Responsibility boundary; IAM user vs. role vs. group; "
    ...
    step("02", "LABS: flaws.cloud — Levels 1–6", "Week 19–20", [
        "Create a free AWS account if you do not have one. The CTF runs against real AWS "
        "infrastructure — you need the AWS CLI installed and configured with a free-tier account",
        "Complete all six levels. Exploit each level fully before reading the solution",
        "For each level: write standard documentation — what the misconfiguration was, how an "
        "attacker exploits it, how to fix it, and what to look for in a code or configuration "
        "review to catch it",
    ])
    step("03", "LABS: flaws2.cloud — Attacker & Defender Paths", "Week 20", [
        "Two distinct paths: attacker (find and exploit misconfigurations) and defender (harden "
        ...
        "Key concepts reinforced: IAM role scoping, S3 bucket policy syntax, EC2 instance metadata "
    ...
    concept("Credential exposure in git history",
            ...
            "permanently visible in history. TruffleHog scans history. Rotation is the fix, not "
            "deletion. The fix is rotation of the exposed credential, not deletion, plus "
            "pre-commit scanning to prevent recurrence.")
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("VI", "Secure Code Review & Threat Modeling", "Weeks 21–23 · Month 5–5.75"))
    add(SP(10))
    ...
    res("OWASP Application Security Verification Standard (ASVS) v4.0",
        "owasp.org/www-project-application-security-verification-standard — Approximately 350 "
        "testable security requirements across 14 domains covering authentication, access "
    ...
    res("Threat Modeling: Designing for Security", "Adam Shostack, Wiley, 2014 (wiley.com) — The "
    ...
    add(SP(4))
    SS("Secure Code Review & Threat Modeling — Weeks 21–23")
    step("01", "READ + PRACTICE: Secure Code Review Methodology", "Weeks 21–22", [
        "Read OWASP Code Review Guide Ch. 1–4 (methodology, code review techniques, "
        "vulnerability-specific review guidance)",
        "Conduct a code review of a real open-source Python/FastAPI application from GitHub — not "
        "WebGoat, but a real project at real scale",
        "Apply the systematic methodology: trace user input, verify authentication, check "
        "authorization, audit crypto, check dependencies",
    ])
    step("02", "STUDY: OWASP ASVS — Verification Standard", "Week 22", [
        "Read ASVS v4.0 Level 1 and Level 2 requirements. Do not try to memorize — learn the "
        "structure and the categories",
        "Map ASVS sections to the vulnerability classes studied in this plan: ASVS V2 is "
        "authentication, V4 is access control, V5 is input validation, V6 is cryptography",
    ...
    step("03", "READ + PROJECT: Threat Modeling", "Weeks 22–23", [
        "Read Threat Modeling: Designing for Security Ch. 1–5",
        "Build a complete threat model for the FastAPI API built during pre-study. A system built "
        ...
        "Produce a threat model document: DFD, trust boundaries, threat enumeration, risk rating, "
        "mitigations",
    ])
    step("04", "PRACTICE: Code Review Under Time Pressure", "Week 23", [
        ...
        "Use GitHub Security Lab’s published CVE writeups as answer keys: read the vulnerable code "
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("VII", "CompTIA Security+ Certification", "Weeks 24–26 · Month 6–6.5"))
    add(SP(10))
    ...
    step("01", "READ: Study Guide", "Weeks 24–25", [
        ...
        "Full notes + note cards: One card per domain. Front: domain name. Back: the key concepts "
        "from that domain most likely to appear on the exam",
    ])
    step("02", "PRACTICE: Practice Exams", "Week 25–26", [
    ...
    step("03", "EXAM: CompTIA Security+ SY0-701", "Week 26", [
    ...
    concept("Security+ exam domains",
            "All five SY0-701 domains: General Security Concepts, "
    ...
    # ════════════════════════════════════════════════════════════════════════
    #  PHASE VIII — TECHNICAL INTERVIEW PREPARATION
    # ════════════════════════════════════════════════════════════════════════
    def qa(q, a):
        add(KeepTogether([
            P(f"<b>Q: {q}</b>", S["bul"]),
    ...
    add(PageBreak())
    add(banner("VIII", "Technical Interview Preparation", "Month 6.5–7"))
    add(SP(10))
    add(P("Before active interviewing begins, confirm the portfolio is in presentation state. "
          "Everything built throughout this plan should be visible, documented, and shareable.",
          S["body"]))
    add(SP(2))
    SS("Pre-Interview Portfolio Checklist")
    add(B("Every GitHub README explains methodology and what the artifact demonstrates — not just "
          "usage instructions"))
    add(B("Juice Shop assessment report is PDF-formatted and shareable as a standalone document"))
    add(B("LinkedIn profile updated: Security+ and BSCP listed, portfolio projects linked, AppSec "
          "transition narrative in the About section"))
    add(B("Minimum 4 OWASP Top 10 writeups published on LinkedIn by this point"))
    add(B("GitHub profile: pinned repos are the Juice Shop report, the Semgrep rule, and the "
          "threat model"))
    add(SP(2))
    add(P("AppSec interviews test security knowledge, adversarial reasoning, and communication — "
          "not algorithmic problem solving. The note card methodology from throughout this plan is "
          "the preparation. This phase adds structured practice in the specific formats that "
          "AppSec interviews use.", S["body"]))
    add(SP(2))
    SS("Interview Format: What to Expect")
    add(B("<b>Secure code review exercise</b> — The most common AppSec technical screen. A code "
          "snippet in Python, JavaScript, or Java with embedded vulnerabilities. 30–45 minutes. "
          "Verbal explanation of findings required."))
    add(B("<b>Conceptual security questions</b> — OWASP concepts, vulnerability explanations, "
          "defensive recommendations. No code required. Note cards cover these."))
    add(B("<b>System design with security focus</b> — Design a secure authentication system, or "
          "review a proposed architecture for security concerns. Developer background is the "
          "advantage here."))
    add(B("<b>Light scripting</b> — Some companies include a Coderpad session: write a Python "
          "script to parse a log file, find a bug in a function, or automate a security check. Not "
          "LeetCode. Your Python background handles this."))
    add(B("<b>Behavioral</b> — How do you handle a developer who pushes back on a finding? How do "
          "you prioritize a backlog of vulnerabilities? How do you explain risk to a non-technical "
          "stakeholder?"))
    add(SP(4))
    SS("Conceptual Questions — Know These Cold")
    qa("Explain SQL injection and demonstrate a prevention.",
       "Parameterized queries, not input sanitization. Show the vulnerable and safe code pattern. "
       "Explain why string concatenation is the root cause.")
    qa("What is the difference between stored and reflected XSS?",
       "Stored persists in the database and fires for all users who load the page. Reflected "
       "requires tricking a specific user into clicking a crafted link. DOM XSS lives entirely in "
       "the client.")
    qa("How does SSRF work and where would you look for it in an API?",
       "Any endpoint that fetches a user-supplied URL server-side. Cloud metadata endpoints "
       "(169.254.169.254) are the canonical exploitation target. Mitigation: URL allowlists, "
       "block internal IP ranges.")
    qa("What does IDOR stand for and give an example.",
       "Insecure Direct Object Reference. Example: /api/invoice/1234 returns data without "
       "checking if the authenticated user owns invoice 1234. Most common access control "
       "vulnerability in production.")
    qa("How would you integrate security into a CI/CD pipeline?",
       "SAST (Semgrep) on every PR, dependency scanning (Snyk) on every build, DAST against "
       "staging, secrets scanning on commits. Fail the build on high-severity findings.")
    qa("A developer pushes back on your XSS finding. How do you respond?",
       "Reproduce the finding with a concrete proof of concept, explain the business risk, offer "
       "to pair on the fix. Adversarial relationships with developers are the primary failure mode "
       "of AppSec programs.")
    qa("Walk me through how you would threat model a new microservice.",
       "Draw the DFD, identify trust boundaries, apply STRIDE to each component, rate each threat "
       "by likelihood and impact, document mitigations. Output is a threat model document.")
    qa("You find a password stored as an MD5 hash in a code review. Walk me through your response.",
       "MD5 is broken for password storage — it is fast by design and easily cracked with rainbow "
       "tables. Flag as high severity. The fix is bcrypt, Argon2, or scrypt with a work factor. In "
       "your report: include the file and line number, a code example of the fix, and note that if "
       "this is a live system, existing hashed passwords cannot be migrated without a reset flow. "
       "Connect it to ASVS V2.4 (credential storage requirements) to frame it against a recognized "
       "standard.")
    qa("What is OWASP ASVS and how would you use it in a code review?",
       "The Application Security Verification Standard — roughly 350 testable security "
       "requirements across 14 domains: authentication, session management, access control, input "
       "validation, cryptography, and more. In a code review you use it as a systematic checklist "
       "rather than relying on memory. Level 1 covers basics automatable by scanners; Level 2 "
       "requires manual judgment. In practice: when you flag a JWT issue, cite ASVS V2.10. When "
       "you flag a missing authorization check, cite ASVS V4.1. It turns ‘this seems wrong’ into "
       "‘this fails ASVS V4.1.1’ — a significant credibility upgrade when communicating findings "
       "to developers or management.")
    SS("Secure Code Review Practice")
    add(P("For interview preparation, practice the following pattern until automatic: read the "
          "code, identify the vulnerability class, explain the exploitation, recommend the fix. Do "
          "this verbally, out loud. Interviews require spoken fluency. Use GitHub Security Lab’s "
          "published CVE writeups as a source of real code examples for cold practice. Five cold "
          "reviews per week during this phase.", S["body"]))
    add(B("<b>Python:</b> FastAPI endpoint accepting user input without sanitization — identify "
          "the injection class"))
    add(B("<b>JavaScript:</b> React component rendering props with dangerouslySetInnerHTML — "
          "identify the XSS vector"))
    add(B("<b>Python:</b> JWT decoded with algorithm=‘none’ accepted — identify the authentication "
          "bypass"))
    add(B("<b>Python:</b> User ID taken from request parameter without authorization check — "
          "identify the IDOR"))
    add(B("<b>Python:</b> URL parameter fetched server-side without validation — identify the "
          "SSRF"))
    add(SP(2))
    SS("Framing Your Background")
    add(B("<b>The narrative:</b> Five years of full-stack software engineering. Deliberate "
          "transition into application security since May 2026. Chose foundational independent "
          "study over bootcamp shortcuts."))
    add(B("<b>The gap:</b> ‘I have been in a full-time transition into application security since "
          "May 2026. Here is what that looks like.’ Share specific projects. The portfolio is the "
          "evidence."))
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("IX", "Expected First Days on the Job", "Month 6+"))
    add(SP(10))
    ...
    books = [
        ...
        [P("Alice and Bob Learn Application Security", S["tc"]), P("Tanya Janca, 2020", S["tc"]),
         P("II–III", S["tcc"]),
         P("wiley.com. The most accessible AppSec entry point in print. SDLC security, threat "
           ...
           "list.", S["tc"])],
        [P("Real-World Bug Hunting", S["tc"]), P("Yaworski, 2019", S["tc"]), P("I–III", S["tcc"]),
         P("nostarch.com. Most accessible intro to web vulns. Organized by vuln class with real "
           "bug bounty reports.", S["tc"])],
        [P("The Web Application Hacker’s Handbook", S["tc"]),
         ...
         P("Dafydd Stuttard and Marcus Pinto, Wiley, 2011 (wiley.com). Ch. 1–3 cover "
           ...
           S["tc"])],
        [P("Hacking APIs", S["tc"]), P("Corey Ball, 2022", S["tc"]), P("III–IV", S["tcc"]),
         P("nostarch.com. REST API security testing. Direct overlap with FastAPI background.",
           S["tc"])],
        [P("Threat Modeling: Designing for Security", S["tc"]), P("Shostack, 2014", S["tc"]),
         P("V", S["tcc"]),
        ...
        [P("TryHackMe", S["tc"]), P("—", S["tcc"]), P("I–II", S["tcc"]),
         P("tryhackme.com. Browser-based guided rooms. Web Fundamentals and Jr Penetration Tester "
           "paths.", S["tc"])],
    ...
    pp = [
        ...
        ("Phase IV — Security Testing & Tooling",
         ...
         "Have it explain Burp Suite Repeater output when a response is unexpected. Ask it to help "
        ...
        ("Phase V — Code Review & Threat Modeling",
         "Use Claude as a review partner: paste a code snippet and ask it to identify "
