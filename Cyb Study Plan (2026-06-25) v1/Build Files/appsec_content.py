...
def build_content(story, S, banner, bi, nb, rule, grid_table,
    ...
    toc = [
        ...
        ("VIII \u2014 Interview Preparation & Interviewing", "42",
            ["Interview Preparation", "Framing Your Background"]),
    ...
    add(B("<b>Legacy PHP, raw SQL, and NoSQL</b> \u2014 You have worked in poorly written legacy "
          "PHP and hand-rolled MySQL \u2014 where SQL injection, IDOR, and unsanitized input "
          "actually live \u2014 and refactored \u2018big ball of mud\u2019 procedural PHP into "
          "cleaner OOP. That is the remediation conversation from both sides: you recognize the "
          "vulnerable pattern and you know how to fix it. DynamoDB experience extends the same "
          "instincts to NoSQL injection and cloud data-access control."))
    add(B("<b>Architecture &amp; domain modeling</b> \u2014 You designed microservices and "
          "domain-driven monolith services with deliberate separation of concerns, trust "
          "boundaries, and loose coupling. Threat modeling is that same structural thinking "
          "turned toward attack: mapping components and data flows, then asking where trust is "
          "granted or assumed."))
    ...
    add(P("This plan is calibrated for 5\u20137 months at 21 study hours per week. 5 "
    ...
    sched = [
        ...
        [P("Full Rest", S["tc"]), P("2", S["tcc"]), P("\u2014", S["tcc"]),
    ...
    names = ["Pre-\nStudy","AppSec\nIntro","Web &\nNetwork","Core\nVulns","Testing &\nTooling",
             "Cloud\nSecurity","Code\nReview","Sec+\nCert","Interview\nPrep","First\nDays"]
    weeks = ["Wks 1\u20133","Wks 4\u20135","Mo 1.5\u20132","Mo 2.5\u20133.5","Mo 3.5\u20134.5",
             "Mo 4.5\u20135","Mo 5\u20135.75","Mo 6\u20136.5","Mo 6.75\u20137.25","Mo 7+"]
    ...
    add(B("<b>GitHub:</b> Scripts from the tooling phase, custom Semgrep rules, automation tools. "
          "Commit history from Month 2 onward. Every README explains the methodology and what the "
          "artifact demonstrates \u2014 not just usage instructions."))
    add(SP(2))
    ...
    step("04", "SETUP", "Git & GitHub Hygiene \u00b7 Day 1", [
        ...
        "into application security as of May 2026. Certifications and portfolio links get added "
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("II", "Web & Network Foundations", "Months 1.5\u20132"))
    add(SP(10))
    ...
    add(SP(4))
    SS("Web & Network Foundations \u2014 Months 1.5\u20132")
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("IV", "Security Testing & Tooling", "Months 3.5–4.5"))
    add(SP(10))
    ...
    res("Gitleaks", "github.com/gitleaks/gitleaks — Lightweight secrets scanner. Runs as a "
        ...
        "for every PR.")
    add(SP(4))
    SS("Security Testing & Tooling — Months 3.5–4.5")
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("V", "Cloud Security Fundamentals", "Months 4.5–5"))
    add(SP(10))
    ...
    add(SP(4))
    SS("Cloud Security Fundamentals — Months 4.5–5")
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("VI", "Secure Code Review & Threat Modeling", "Months 5–5.75"))
    add(SP(10))
    ...
    add(SP(4))
    SS("Secure Code Review & Threat Modeling — Months 5–5.75")
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("VII", "CompTIA Security+ Certification", "Months 6–6.5"))
    add(SP(10))
    ...
    add(PageBreak())
    add(banner("VIII", "Interview Preparation & Interviewing", "Months 6.75–7.25"))
    add(SP(10))
    ...
    add(B("<b>Secure code review:</b> practice the read \u2192 identify the vulnerability class "
          ...
          "do it out loud \u2014 interviews require spoken fluency. Use GitHub Security Lab\u2019s "
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("IX", "Expected First Days on the Job", "Months 7+"))
    add(SP(10))
