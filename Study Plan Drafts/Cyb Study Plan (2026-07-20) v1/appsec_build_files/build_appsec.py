...
def main():
    ...
    # ── Prose TIMELINE-REFERENCE guard (runs on every GENERATE PDF) ──────────
    # report_only()/reconcile() above validate the banner/chart/schedule/step machinery.
    # They do NOT read paragraphs or callout boxes, where hard-coded months/weeks go stale
    # when a phase moves. This surfaces those (judgment to fix, so it WARNS).
    print("Checking phase month-boundaries...")
    import timeline_boundary_check
    if timeline_boundary_check.check(CONTENT_PATH):
        print("** WARNING: phase boundary/gap issue(s) above \u2014 fix before relying on this build.\n")
    else:
        print("Phase boundaries contiguous. \u2713\n")

    print("Checking prose timeline references...")
    import timeline_ref_check
    if timeline_ref_check.check(CONTENT_PATH):
        print("** WARNING: stale prose timeline reference(s) above \u2014 fix before relying on this build.\n")
    else:
        print("No stale prose timeline references. \u2713\n")
