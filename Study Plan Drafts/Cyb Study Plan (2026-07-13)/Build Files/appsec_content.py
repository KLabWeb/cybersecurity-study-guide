...
def build_content(story, S, banner, bi, nb, rule, grid_table,
    ...
    names = ["Pre-\nStudy","AppSec\nIntro","Web &\nNetwork","Core\nVulns","Testing &\nTooling",
             "Cloud\nSecurity","Code\nReview","Sec+\nCert","Interview\nPrep","First\nDays"]
    weeks = ["Wks 1\u20133","Wks 4\u20135","Mo 1.5\u20132.25","Mo 2.5\u20134.0","Mo 4.0\u20136.25",
             "Mo 6.25\u20136.75","Mo 6.75\u20137.5","Mo 7.75\u20138.5","Mo 8.75\u20139.25","Mo 9.0+"]
    ...
    for _line in [
        ...
        "<b>Phase III — Core Vulnerability Classes</b> (Mo 2.5–4.0). <b>Purpose:</b> master the core web vulnerability classes hands-on at practitioner depth. <b>Resources:</b> PortSwigger topic paths and their apprentice/practitioner labs, “Hacking APIs”, the OWASP Top 10. <b>Achievement:</b> practitioner labs across the OWASP classes and an OWASP Top 10 writeup series.",
        "<b>Phase IV — Security Testing & Tooling</b> (Mo 4.0–6.25). <b>Purpose:</b> tooling fluency — Burp mastery, SAST, DAST, SCA — and the BSCP certification. <b>Resources:</b> Burp docs, PortSwigger BSCP practice exams, Semgrep, Bandit, OWASP ZAP, Snyk. <b>Achievement:</b> passed the Burp Suite Certified Practitioner (BSCP) exam, completed a full Juice Shop security assessment.",
        ...
        "<b>Phase VI — Secure Code Review & Threat Modeling</b> (Mo 6.75–7.5). <b>Purpose:</b> the core AppSec craft — manual secure code review and threat modeling. <b>Resources:</b> OWASP Code Review Guide, OWASP ASVS, “Threat Modeling: Designing for Security”. <b>Achievement:</b> a documented code review and a threat model project.",
        "<b>Phase VII — CompTIA Security+ Certification</b> (Mo 7.75–8.5). <b>Purpose:</b> earn the credential that clears HR and ATS filters; main application push. <b>Resources:</b> the “Get Certified Get Ahead: SY0-701” study guide and Professor Messer practice exams. <b>Achievement:</b> passed CompTIA Security+ (SY0-701).",
        "<b>Phase VIII — Interview Preparation & Interviewing</b> (Mo 8.75–9.25). <b>Purpose:</b> convert skills and portfolio into offers — interview prep, story framing, application strategy. <b>Resources:</b> portfolio writeups and mock interviews. <b>Achievement:</b> a polished portfolio and interview narrative, plus active interviewing.",
        "<b>Phase IX — Expected First Days on the Job</b> (Mo 9.0+). <b>Purpose:</b> start strong in the new sub-field and role and know your immediate differentiators. <b>Resources:</b> your accumulated notes and the five core reference books. <b>Achievement:</b> a written 30/60/90-day onboarding plan and early on-the-job wins.",
    ]:
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("III", "Core Vulnerability Classes", "Months 2.5–4.0"))
    add(SP(10))
    ...
    add(SP(4))
    SS("Core Vulnerability Classes — Months 2.5–4.0")
    ...
    step("08", '<a name="p3_llm_attacks"/>READ + LABS: Web LLM Attacks (PortSwigger)', "Weeks 15\u201316", [
    ...
    step("08", "READ: OWASP Top 10 for LLM Applications", "Week 16", [
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("VII", "CompTIA Security+ Certification", "Months 7.75–8.5"))
    add(SP(10))
    add(P("CompTIA Security+ is best understood as a resume-screen credential — that is the honest framing. It is required or preferred "
    ...
    add(PageBreak())
    add(banner("VIII", "Interview Preparation & Interviewing", "Months 8.75–9.25"))
    add(SP(10))
    add(P("AppSec interviews test security knowledge, adversarial reasoning, and communication "
          "\u2014 not algorithmic problem solving. Prep splits into two tracks. Track A is pure memorization and rehearsal \u2014 behavioral "
          "answers, past-job stories, and the foundational AppSec question-and-answer set \u2014 "
          "delivered aloud until it is natural. Track B is different: the live, watched formats "
          "(secure code review out loud, light scripting) are a performance skill, and they fail "
          "on nerves, not knowledge, so Track B is trained by desensitization \u2014 timed, watched "
          "reps at volume, spread across months and started early, not crammed here. Both tracks, "
          "and how each maps to the interview formats, are covered below.", S["body"]))
    add(SP(2))
    SS("Track A: Behavioral Question Rehearsals")
    ...
    add(SP(2))
    SS("Track A: Technical Note Cards")
    ...
    add(B("<b>Foundational AppSec questions:</b> alongside cards drawn from your own study, seed "
          "a deck from a curated interview-question bank such as <a href=\"https://github.com/jassics/security-interview-questions\" color=\"#3A7AB8\">jassics/security-interview-questions</a> "
          "so you rehearse the questions that actually get asked, in your own words."))
    ...
    add(SP(2))
    SS("Track B: Technical Programming Practice")
    ...
    add(B("<b>Python:</b> a URL parameter fetched server-side without validation \u2014 identify "
          "the SSRF"))
    add(SP(2))
    SS("Track B: Desensitizing the Live Formats (Start Early)")
    add(P("The point of Track B is exposure, not learning \u2014 make the watched, timed format so "
          "routine that it stops triggering the freeze. Do not save this for Month 8.75: begin as "
          "soon as you can read vulnerable code (Phase III), keep it running through every later "
          "phase, and have mock interviews underway well before your first real interview.", S["body"]))
    add(B("<b>Graded exposure ladder:</b> climb one rung at a time \u2014 solo and untimed, then solo "
          "on a timer, then recorded and played back, then watched live by a mentor or peer, then "
          "watched by a stranger. Move up only once the current rung stops spiking your anxiety."))
    add(B("<b>Rehearse the recovery, not just the solve:</b> practice narrating your thinking out "
          "loud, and drill the explicit \u2018I am not certain yet \u2014 here is how I would approach "
          "it\u2019 move, so a blank moment becomes a spoken plan instead of a silent lock-up. That "
          "recovery line is the single most valuable thing to have automatic."))
    add(B("<b>Mock cadence:</b> a couple of watched or timed reps each week from Phase III onward, "
          "ramping to a full mock (behavioral plus code review) roughly monthly once applications "
          "begin at Month 4. Never let a real interview be the first time you perform the format cold."))
    add(B("<b>Where to run mocks:</b> <a href=\"https://mentorcruise.com/\" color=\"#3A7AB8\">MentorCruise</a> "
          "for AppSec-specific mentor mocks (code review plus calibration); <a href=\"https://www.tryexponent.com/\" color=\"#3A7AB8\">Exponent</a>\u2019s "
          "Security Engineering track for structured security mocks; and <a href=\"https://interviewing.io/\" color=\"#3A7AB8\">interviewing.io</a> "
          "for cheap, high-volume behavioral and coding-under-observation reps (general, not "
          "AppSec-specific \u2014 use it for the desensitization volume)."))
    add(SP(2))
    ...
    # ════════════════════════════════════════════════════════════════════════
    add(PageBreak())
    add(banner("IX", "Expected First Days on the Job", "Months 9.0+"))
    add(SP(10))
