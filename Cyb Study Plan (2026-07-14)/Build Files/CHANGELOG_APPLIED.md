...
## Batch: FIX phantom phase-gaps (off-by-one start months) + new boundary check (BUILD)  [2026-07-14]
- Root cause: phase banner START months were computed as first_week/4 in places, but the START of week A
  is (A-1)/4 (= the END of the prior week). first_week/4 invents a phantom 0.25-month gap between phases.
  report_only() only validated END coverage, never START/contiguity, so it slipped through.
- Corrected START months (banner + chart + schedule-SS + summary, all reps): II 1.5->1.25, III 2.5->2.25,
  VII 7.75->7.5, VIII 8.75->8.5, IX 9.0->9.25. (IV/V/VI were already correct/contiguous.) All phases now
  share boundaries with no gaps or overlaps. Dependent prose (Track B "save this for Month 8.75"->"8.5") updated.
- NEW check timeline_boundary_check.py: verifies each month-phase START=(first_week-1)/4, END=last_week/4,
  and full-chain contiguity (covers VIII/IX which have no week steps). Wired into build_appsec.py -> runs on
  every GENERATE PDF. This is the guard report_only never had.
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0; render-path OK. NOT rendered.

## Batch: IMPLEMENT items 66-68 (BUILD, source only)  [2026-07-14]
66 removed " — that is the honest framing" from the Phase VII intro (now "...resume-screen credential. It is required...").
67 Phase VII schedule heading "CompTIA Security+ Certification Schedule" -> "CompTIA Security+ Certification — Months 7.5–8.5"
   (months derived from phase weeks 31-34), matching the other phases' "Title — Months X–Y" format.
68 Italicized the only non-italic book title: "CompTIA Security+ Get Certified Get Ahead: SY0-701 Study Guide" in both
   its Learning-Resources res() entry and step 01, with curly quotes to match every other book-title reference.
   (All other book titles, incl. abbreviated WAHH, were already italicized.)
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0; render-path OK. NOT rendered.

**81:** "Study Plan Phase Summary" still breaks book-title formatting by surrounding book titles in quotes instead of no-quotes + italicization. Fix.

## Batch: IMPLEMENT items 69-71 + GENERATE  [2026-07-14]
69 "build note cards in book order" -> "label and study note cards in book order" (Phase VII step 01 card bullet).
70 "learn cards in groups of seven to full front-to-back recall" -> "memorize cards in groups of seven with full front-to-back recall".
71 "re-drill the whole pile as it grows" -> "re-drill the whole pile each time you add a new memorized 7 card group to it".
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0; render-path OK.

## Batch: IMPLEMENT items 72-74 + GENERATE  [2026-07-14]
72 "at ~200 cards" -> "at ~180 cards" (Phase VII memorize bullet).
73 "Study weak domains specifically from the study guide" -> "...via note cards and additional learning, if required".
74 "Two or three cycles is sufficient" -> "Repeat until score 85–90% over 2–3 practice exams in a row" (Phase VII step 02).
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0; render-path OK.

## Batch: IMPLEMENT items 75, 77, 78 + GENERATE  [2026-07-14]
75 EXAM step: replaced the price bullet with 3 bullets -- Delivery (in-person at a hyperlinked "Center City
   Philadelphia test center" -> Pearson VUE CompTIA locator; no online option mentioned), Scheduling (book soonest
   seat at 85-90%, do not delay), Retakes (unlimited; no wait 1st retake, 14-day after 2nd+ fail; 12-mo lock after
   pass). Price updated $392 -> $404 here AND in the Appendix Certifications table for consistency.
77 APPLY: Main Application Push: added "Daily target" bullet (2-4 apps/day, tailored resume+cover letter, ~1-2 hrs
   each) + a cross-ref bullet "See Application Strategy in Phase VIII for the full targeting, profile, and framing
   playbook" (hyperlinked to #application_strategy).
78 Removed the "By now you should have 4-6 published writeups..." bullet.
NOTE: item 77's new cross-ref bullet now overlaps the existing "Continue through interview preparation... see
   Application Strategy there" bullet -- item 79 (next) rewords that one; will be near-duplicate, flag to consolidate.
NOTE: Appendix Certifications table lists Security+ as Phase VI (should be VII) -- spotted, not on changelist, untouched.
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0; render-path OK.

**82:** Remove bullet "List on resume as: CompTIA Security+ SY0-701 (Expires [date])" (Phase VII EXAM step).

**83:** Fix bug: the Appendix "Certifications" table lists Security+ as Phase VI; it should be Phase VII.

## Batch: IMPLEMENT items 79, 80, 81 + GENERATE  [2026-07-14]
79 Removed the duplicate "Continue through interview preparation... see Application Strategy there" bullet from the
   Phase VII APPLY step (option A -- item 77's cross-ref bullet is the single remaining one).
80 Phase VIII restructure: rewrote the intro to define two FOCUSES -- <b>Memorization</b> and <b>Live Practice</b>
   (bulleted, "focuses" not "tracks"). Converted the four focus subsections to sequential numbered step-headers
   (1. Memorization -- Behavioral Question Rehearsals; 2. Memorization -- Technical Note Cards;
   3. Live Practice -- Technical Programming Practice; 4. Live Practice -- Desensitizing the Live Formats),
   matching the "N. Label: tag" step format of Phases I-VII, with all content/links preserved beneath.
   Renamed the body "Track B" reference to "Live Practice". Reference subsections (Interview Preparation, Framing
   Your Background, Application Strategy) left as SS reference blocks, consistent with other phases.
   [BUG CAUGHT mid-build: intro-splice anchor matched an earlier identical phrase (L205) -- fixed to unique anchor
    before any write; nothing corrupted.]
81 "Study Plan Phase Summary": book titles in the per-phase Resources lists changed from curly-quotes to no-quotes +
   italics (Real-World Bug Hunting, Alice and Bob, Threat Modeling, Get Certified Get Ahead), and WAHH italicized.
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0; render-path OK.

## Batch: IMPLEMENT items 82, 83 + GENERATE  [2026-07-14]
82 Removed the "List on resume as: CompTIA Security+ SY0-701 (Expires [date])" bullet from the Phase VII EXAM step.
83 Fixed Appendix "Certifications" table: Security+ row Phase VI -> VII.
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0; render-path OK.
CHANGELIST NOW EMPTY (items 66-83 done; 76 withdrawn).

## Batch: Phase VIII restyle to house format + GENERATE  [2026-07-14]
- Added the missing schedule header + rule: SS("Interview Preparation & Interviewing — Months 8.5–9.25"), placed
  after the intro and before the steps, matching every other phase.
- Converted the four focus subsections from manual paragraph-headers to real bulleted step() calls (01-04), so the
  phase is bullet-based and organized like Phases I-VII. Step labels now use ":" not "—"
  (Memorization: Behavioral Question Rehearsals; Memorization: Technical Note Cards;
   Live Practice: Technical Programming Practice; Live Practice: Desensitizing the Live Formats (Start Early)).
  Long intro paragraphs condensed into lead bullets; the 5 cold-review snippets folded into one drill bullet.
  All hyperlinks preserved (jassics bank, GitHub Security Lab, MentorCruise, Exponent, interviewing.io) + the
  Impractical Python Projects book italic.
- Renamed the reference "Interview Preparation" block to "Mapping Formats to Methods" to avoid colliding with the
  new schedule header. Framing Your Background and Application Strategy reference blocks kept (already bulleted).
- Phase VIII steps now carry week tags (01-03 Weeks 35-37; 04 From Phase III onward); boundary check validates
  VIII = Months 8.5-9.25 from those, still contiguous.
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0; render-path OK.

## Batch: IMPLEMENT items 84-90 + GENERATE  [2026-07-14]
85 "then rehearse aloud" -> "and rehearse aloud" (P8 step 01).
86 removed the "toughest challenge / disagreed with a teammate / why this transition" examples clause (P8 step 01).
87 removed "Keep a tight and an expanded version of each to scale to follow-ups" (P8 step 01).
88 note-card topics fixed to real cyber topics: OWASP Top 10 + web vuln classes, Security+ domains, PortSwigger
   attack techniques, defensive/secure-coding patterns (dropped Python/SQL/db-normalization) (P8 step 02).
89 "seed a deck from a curated bank" -> "create a deck from a reputable existing set of cybersecurity interview questions".
90 "5 passes each" -> "3-5 passes each" (P8 step 02 review method).
84 SEQUENTIAL-ORDER FIX: the desensitization activity (was a Phase VIII step tagged "From Phase III onward") now has
   its home step in Phase III where it starts -- new Phase III step 05 "PRACTICE: Begin Live-Format Desensitization"
   (Throughout Weeks 11-16) holding the full method (ladder, recovery, mock cadence, where-to-run-mocks). Phase VIII
   step 04 kept as a brief continuation "Live Practice: Desensitization (Final Push)" (Weeks 35-37) linking back to it.
   Fixed a PRE-EXISTING dup step-number bug: Phase III had two "08"s (Web LLM + OWASP). ROOT CAUSE: parse()/reorder
   regexes only match double-quoted step labels, so steps whose labels held <a name> anchors (single-quoted) were
   skipped by fix_step_numbering, causing collisions. FIX (no machinery change): moved the anchors out of the labels
   into each step's first bullet, so all labels are double-quoted; Phase III now renumbers cleanly to 01-10 and
   reconcile is stable (changed=False).
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0 hard; render-path OK; reconcile stable.

## Batch: revert item 84's Phase III insertion; desensitization is Phase VIII only + GENERATE  [2026-07-14]
- Removed the Phase III step 05 "Begin Live-Format Desensitization" entirely (user: do this only in interview prep).
- Restored the full desensitization step into Phase VIII (step 04, "Live Practice: Desensitizing the Live Formats",
  Weeks 35-37) from the pre-change (build-20) version, rescoped to Phase VIII: dropped the "start early / from Phase III
  onward / Do not save for Month 8.5" framing and the Month-4 reference; kept the ladder/recovery/mock-cadence/where-to-
  run-mocks method and links.
- Phase III renumbered back to 01-09 (Web LLM 08, OWASP 09); dup-08 stays fixed (anchor-relocation retained), reconcile
  stable (changed=False). Removed the now-orphaned #live_desensitization anchor + link. Items 85-90 retained.
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0 hard; render-path OK.

## Batch: IMPLEMENT items 91-93 (Phase VIII to house style) + GENERATE  [2026-07-14]
93 Added a Phase VIII "Learning Resources" section (after intro, before schedule header) with res() entries matching
   the house format: Impractical Python Projects (italic, no quotes, per user), GitHub Security Lab, the security
   interview question bank (jassics), and the mock-interview platforms (MentorCruise/Exponent/interviewing.io). All
   resource URLs are plain-text (auto-linked by linkify) as in other phases. Consolidated every resource out of the
   step bullets into this section (steps now reference them by name / "see Learning Resources").
91 Rewrote the step 03 "Secure code review" bullet: replaced the vague "drill the read -> ..." with an explicit,
   spoken four-step loop (read code -> name the class -> explain exploitation -> recommend the fix), and clarified how
   to use GitHub Security Lab + CVE writeups (work through them cold, ~5/week) -- resource now lives in Learning Resources.
92 "Build fluency writing correct Python quickly without an IDE" -> "Refresh fluency writing correct Python quickly
   without IDE hints"; moved Impractical Python Projects to Learning Resources.
NOTE: Phase VIII res book title is italic-no-quotes per user's explicit instruction; other phases' res books still use
   curly-quotes+italic -- a doc-wide consistency choice to confirm later.
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0 hard; render-path OK.
CHANGELIST EMPTY (66-93 done; 76 withdrawn).

## Batch: Phase VIII rewrite (clarity, no slang/shorthand, inline hyperlinks) + GENERATE  [2026-07-14]
Full rewrite of the Phase VIII intro, Learning Resources, and four steps for clearer prose, no slang/shorthand,
and inline hyperlinks throughout. Specific edits:
- Intro Live Practice bullet: removed ", spread across months and started early, not crammed here." (new period).
- Learning Resources book now matches house format: "<i>Impractical Python Projects</i>" (curly-quotes+italic) --
  Lee Vaughan, No Starch Press, 2018 (nostarch.com).
- Step 02: removed "the cards are the most load-bearing prep tool"; jassics changed to "...via
  github.com/jassics/security-interview-questions" with the URL hyperlinked; "a couple more times"->"further";
  "reviewed groups"->"reviewed topics".
- Step 03: removed the "hands-on formats...reward deliberate practice" bullet; Secure code review rewritten as an
  explicit spoken routine with GitHub Security Lab hyperlinked inline; Light scripting keeps "Refresh...without IDE
  hints" and hyperlinks Impractical Python Projects to nostarch; "spot the class on sight" -> "Recognition drills".
- Step 04: removed ", then watched by a stranger"; "Rehearse the recovery, not just the solve" -> "Handling moments
  when you are stuck"; "Mock cadence" -> "Schedule of mock interviews" with MentorCruise/Exponent/interviewing.io
  hyperlinked inline.
- Removed all "see Learning Resources" phrasing; resources are named and hyperlinked inline in the steps.
- De-slanged throughout (reps/cold/grind/freeze/lock-up removed); reference blocks scanned clean.
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0 hard; render-path OK.

## Batch: Phase VIII rebuilt to match Phases I-VII (deep-dive) + GENERATE  [2026-07-14]
Deep-dived Phases 0-IX first. Confirmed the III-VII step convention (ACTION-CATEGORY: Subject label + timing tag,
imperative bullets with inline links) and the section pattern (intro prose -> Learning Resources -> schedule header
-> steps -> reference sections). Rebuilt Phase VIII to match:
- Removed the two-track (Memorization / Live Practice) framing entirely, including the two-focus intro bullets.
- Intro is now a single prose paragraph like other phases (no bullets, no "two focuses").
- Steps are now five sequential action-category steps written "do this, using this":
  01 REHEARSE: Behavioral Questions; 02 MEMORIZE: Technical Note Cards; 03 PRACTICE: Secure Code Review;
  04 PRACTICE: Light Scripting & System Design; 05 PRACTICE: Mock Interviews. Imperative bullets, inline hyperlinks
  (jassics bank, GitHub Security Lab, Impractical Python Projects, MentorCruise/Exponent/interviewing.io).
- Specific edits: "Rehearse the AppSec-specific prompts as well:" -> "Rehearse AppSec-specific prompts as well:";
  "...non-technical stakeholder." -> "...non-technical stakeholder, and so forth."; standardized "note-card" -> "note
  card" doc-wide (3 fixes).
- Kept Learning Resources, schedule header, and the reference sections (Mapping Formats to Methods, Framing Your
  Background, Application Strategy) -- Phase VIII's phase-specific reference material, consistent with how Phase 0/IX
  also carry phase-specific tail sections rather than Concepts Mastered.
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0 hard; render-path OK; reconcile stable.

## Batch: IMPLEMENT item 93.5 -- full Phase VIII rewrite (deep-dive) + GENERATE  [2026-07-14]
Did a genuine full read of Phases I, II, VI (end to end) plus every phase's step labels + section headers, then
read Phase VIII in full including the reference blocks previously skipped. Rewrote Phase VIII to match Phases III-VII:
prose intro; Learning Resources; schedule header; five sequential action-category steps (01 REHEARSE: Behavioral
Questions; 02 MEMORIZE: Technical Note Cards; 03 PRACTICE: Secure Code Review; 04 PRACTICE: Light Scripting & System
Design; 05 PRACTICE: Mock Interviews) written "do this, using [linked resource]" like Phase VI; then the reference
sections (Mapping Formats to Methods, Framing Your Background, Application Strategy). No two-track framing.
This pass FOLDED IN changelist items 94-112 (all now handled), so they are removed as no-longer-relevant per 93.5:
  94 dropped stray "and"; 95 "by topic in sub-groups of seven"; 96 "3-5 passes"; 97 "some ... CVE write-ups" +
  linked to GitHub Security Lab advisories; 98 "without IDE hints"; 99 "(ex. CoderPad)"; 100 book italic sans quotes
  in the step; 101 replaced vague "Design and critique architectures" with a concrete verbal system-design drill
  (sketch trust boundaries, apply STRIDE, walk through aloud -- not a written deliverable); 102 Exponent linked to its
  security-engineering course search; 103 removed the mentor/peer/stranger exposure ladder (solo progression only,
  ending at the three mock sites); 104 "methods above" -> "steps above"; 105 generalized the conceptual-questions
  mapping to every carded topic; 106 removed "your developer background is the advantage"; 107 added "the BSCP
  certification acquired"; 108 added #art_juiceshop / #art_semgrep / #art_threatmodel anchors on Phase IV step 04,
  Phase IV step 17, Phase VI step 03 and hyperlinked the three named deliverables back to them; 109 collapsed the
  duplicated transition/gap/portfolio passages; 110 removed the AppSec-adjacent role broadening; 111 collapsed the
  duplicated full-stack pitch; 112 role targeting is now application-security-engineer only.
Also purged stale already-implemented items (45-93) that had been left under old PENDING headers.
Gates: boundary PASS; report_only 0; prose PASS; edit_check PASS; dedup 0 hard; render-path OK; reconcile stable.

## PENDING (newly added, NOT yet implemented)  [2026-07-14]
## Batch: remove Mapping Formats to Methods; LinkedIn meetup note; remove Target-roles bullet [2026-07-14 IMPLEMENTED]
- Removed the entire 'Mapping Formats to Methods' subsection from Phase VIII.
- Appended to the 'Make LinkedIn application-ready' bullet: 'Publish as you work through the plan, so you have work to show off during local cyber security community meetups.'
- Removed the 'Target application security engineer roles -- the focus of this entire plan.' bullet.
## Batch: appendix + Phase IX text edits [2026-07-14 IMPLEMENTED]
- 'The engineering history is the relevant benchmark' -> 'The self-taught web dev engineering history...'.
- 'within the first weeks of employment' -> 'within the first initial weeks of employment' (corrected the 'Intial' typo to 'initial').
- Rebuilt appendix 'Books & Learning Resources': enumerated every res() across all phases via Python, deduped (merged MDN and TryHackMe variants; excluded the two personal-notes entries), sorted by first appearance in the document, one row each -- 41 resources (was 8). URLs taken from the phase Learning Resources (source of truth), plain-text (auto-linked).
- Removed GWEB and CSSLP from Certifications (out of scope).
- Fixed Practical Use Per Phase label drift: 'Phase V -- Code Review...' -> 'Phase VI -- Secure Code Review & Threat Modeling'; 'Phase VI -- Sec+ Certification' -> 'Phase VII -- CompTIA Security+ Certification'; 'Phase VII -- Sec+ & Interviews' -> 'Phase VIII -- Interview Preparation & Interviewing'. Content of each entry verified to match its true phase's steps/intentions.
